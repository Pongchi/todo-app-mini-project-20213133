function TaskDetail({ todo, onDelete, onToggle }) {
  if (!todo) {
    return (
      <div className="w-72 flex-shrink-0 p-6 flex flex-col items-center justify-center bg-white">
  ...
  return (
    <div className="w-72 flex-shrink-0 flex flex-col p-6 bg-white">
      <p className="text-xs font-semibold text-gray-400 mb-2">Task:</p>

      <h2 className={`text-base font-semibold mb-1 leading-snug ${todo.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>
        {todo.title}
      </h2>

      <p className="text-xs text-gray-300 mb-6">No description</p>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-lg">{createdDate}</span>
        </div>
      </div>
